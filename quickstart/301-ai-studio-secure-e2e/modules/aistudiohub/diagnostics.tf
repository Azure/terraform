resource "azurerm_monitor_diagnostic_setting" "diagnostic_setting_ai_studio_hub" {
  ...
  target_resource_id         = azapi_resource.ai_studio_hub.id
  ...
  dynamic "enabled_log" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.diagnostic_categories_ai_studio_hub.log_category_groups
    content {
  ...
  dynamic "metric" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.diagnostic_categories_ai_studio_hub.metrics
    content {
